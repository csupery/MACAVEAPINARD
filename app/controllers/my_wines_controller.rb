class MyWinesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @my_wine.save
        format.html { redirect_to my_wine_url(@my_wine), notice: "My wine was successfully created." }
  ...
  def update
    respond_to do |format|
      if @my_wine.update(my_wine_params)
        format.html { redirect_to my_wine_url(@my_wine), notice: "My wine was successfully updated." }
  ...
  private
    ...
    def my_wine_params
      params.require(:my_wine).permit(:name, :appellation_winery_id, :vintage, :quantity, :size, :my_wine_cellar_id)
    end
end
